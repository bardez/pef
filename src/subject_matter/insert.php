<?php 
    ...
    if(!empty($_POST['Nome'])){
        $nome = $_POST['Nome'];
        $classe_processo = $_POST['classe_processo'];
        $sqlSelect = "SELECT * FROM ASSUNTO_PRINCIPAL WHERE Nome='$nome'";
        $responseSelect = mysqli_query($connection, $sqlSelect);
        $numRowsSelect = mysqli_num_rows($responseSelect);
        if($numRowsSelect> 0)
        {
            echo "Assunto principal '$nome' já cadastrado";
        }
        else {
            $sqlInsert = "INSERT INTO ASSUNTO_PRINCIPAL (Nome, CLASSE_PROCESSO_Codigo) VALUES (?, ?)";
            $cmd = mysqli_prepare($connection, $sqlInsert);
            mysqli_stmt_bind_param($cmd,'si',$nome, $classe_processo);
            ...
            <div class="form-row">
                <div class="col">
                    <div class="form-group">
                        ...
                        <input type="text" name='Nome' class="form-control" id="Nome" placeholder="Digite o assunto">
                    </div>
                </div>
                <div class="col">
                    <div class="form-group">
