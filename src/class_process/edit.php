<?php 
    ...
    if(!empty($_GET['Codigo'])){
        ...
        $nome = $_POST['Nome'];
        $codigo = $_POST['Codigo'];
        $compCodigo = $_POST['competencia'];
        $sqlUpdate = "UPDATE CLASSE_PROCESSO SET Nome = ?, COMPETENCIA_Codigo = ? WHERE Codigo = ?";
        $cmd = mysqli_prepare($connection, $sqlUpdate);
        mysqli_stmt_bind_param($cmd,'sii',$nome, $compCodigo, $codigo);
        mysqli_stmt_execute($cmd);
...
<body>
    ...
    <div>
        <h4 class="custom-form-title">Edição DE Classe de processo</h4>
        <hr>
